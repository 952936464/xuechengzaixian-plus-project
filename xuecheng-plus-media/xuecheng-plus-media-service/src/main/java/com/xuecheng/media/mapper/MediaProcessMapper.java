...
public interface MediaProcessMapper extends BaseMapper<MediaProcess> {
    /**
     *
     * @param shardTotal 一共有几个调度器
     * @param shardIndex 那个调度器调度
     * @param count 查几条
     *              fail_count ：最多失败次数
     * @return
     */
     ...
     * @return 更新记录数
     */
    @Update("update media_process m set m.status = '4' where (m.status = '1' or m.status = '3') and m.id = #{id}")
    int startTack(@Param("id") long id);
}
